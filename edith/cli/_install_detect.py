...
We detect by inspecting ``edith.__file__``. If we can't tell with
...
def detect_install() -> InstallInfo:
    ...
    Cheap: just walks the parent directories of ``edith.__file__``
    once and checks for marker directories. No subprocess calls.
    """
    try:
        import edith

        pkg_file = Path(edith.__file__).resolve()
    except Exception:
        return InstallInfo(
            kind="unknown",
