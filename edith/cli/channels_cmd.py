...
def channels_status() -> None:
    ...
    from edith.channels.imessage_daemon import is_running
    ...
    if background:
        ...
        proc = subprocess.Popen(
            [
                sys.executable,
                "-m",
                "edith.channels.imessage_daemon",
                "--chat",
                chat_identifier,
            ],
        ...
        from edith.connectors.retriever import (
            TwoStageRetriever,
        )
        from edith.connectors.store import KnowledgeStore
        from edith.engine.ollama import OllamaEngine
        from edith.tools.knowledge_search import (
            KnowledgeSearchTool,
        )
        from edith.tools.knowledge_sql import (
            KnowledgeSQLTool,
        )
...
def imessage_stop() -> None:
    ...
    from edith.channels.imessage_daemon import stop_daemon
