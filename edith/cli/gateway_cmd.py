...
def start(install: bool) -> None:
    ...
    if install:
        ...
        from edith.daemon.service import (
        ...
        if plat.system() == "Darwin":
            plist_path = (
                Path.home() / "Library/LaunchAgents/com.edith.gateway.plist"
            )
...
def stop() -> None:
    ...
    if plat.system() == "Darwin":
        subprocess.run(
            ["launchctl", "remove", "com.edith.gateway"],
            check=False,
        )
    else:
        subprocess.run(
...
def status() -> None:
    ...
    if plat.system() == "Darwin":
        subprocess.run(
            ["launchctl", "list", "com.edith.gateway"],
            check=False,
        )
    else:
        subprocess.run(
...
def logs() -> None:
    ...
    if plat.system() == "Darwin":
        click.echo("Check ~/Library/Logs/com.edith.gateway.log")
    else:
        subprocess.run(
