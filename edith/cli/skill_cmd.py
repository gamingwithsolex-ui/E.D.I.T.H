...
def _get_resolver(source: str, url: str = ""):
    ...
    if source == "hermes":
        from edith.skills.sources.hermes import HermesResolver

        return HermesResolver()
    if source == "openclaw":
        from edith.skills.sources.openclaw import OpenClawResolver
...
def install(query: str, with_scripts: bool, force: bool, url: str):
    ...
    from edith.skills.importer import SkillImporter
    from edith.skills.parser import SkillParser
    from edith.skills.tool_translator import ToolTranslator
