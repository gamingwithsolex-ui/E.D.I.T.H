...
from edith.cli._banner import print_banner
from edith.core.config import load_config
from edith.core.events import EventBus
from edith.engine import (
    discover_engines,
    discover_models,
    get_engine,
)
from edith.intelligence import (
...
def serve(
    ...
    if _has_cloud and engine_name != "cloud":
        try:
            from edith.engine.cloud import CloudEngine
            from edith.engine.multi import MultiEngine
    ...
    try:
        from edith.telemetry.instrumented_engine import InstrumentedEngine
    ...
    if agent_key:
        try:
            import edith.agents  # noqa: F401
            from edith.core.registry import AgentRegistry
            ...
            if AgentRegistry.contains(agent_key):
                ...
                if getattr(agent_cls, "accepts_tools", False):
                    import edith.tools  # noqa: F401  # trigger registration
    ...
    if channel_bridge is not None:
        from edith.system import JarvisSystem
    ...
    # Create app
    from edith.server.app import create_app
    ...
    if config.agent.context_from_memory:
        try:
            import edith.tools.storage  # noqa: F401
    ...
    from edith.server.auth_middleware import check_bind_safety

    check_bind_safety(bind_host, api_key=api_key)

    # Log credential status at startup
    from edith.core.credentials import TOOL_CREDENTIALS, get_credential_status
