...
class WorkflowEngine:
    ...
    def _run_agent_node(
        ...
        try:
            ...
            output_val = result if isinstance(result, str) else result.get("content", "")
            return WorkflowStepResult(
                node_id=node.id,
                success=True,
                output=output_val,
    ...
    def _run_tool_node(
        ...
        if system and system.tool_executor:
            from edith.core.types import ToolCall
    ...
    def _run_condition_node(
        ...
        try:
            if "==" in expr:
                ...
                val = outputs.get(left, left)
                result = "true" if val == right.strip("'\"") else "false"
            elif " in " in expr:
                left, right = [x.strip() for x in expr.split(" in ", 1)]
                val = outputs.get(right, right)
                result = "true" if left.strip("'\"") in val else "false"
            else:
                result = "true" if outputs.get(expr, expr) else "false"
        except Exception:
            result = "false"
        return WorkflowStepResult(
            node_id=node.id,
    ...
    def _run_loop_node(
        ...
        for i in range(max_iter):
            ...
            if system:
                ...
                last_output = result if isinstance(result, str) else result.get("content", "")
