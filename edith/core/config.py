...
if TYPE_CHECKING:
    ...
    # top level.
    from edith.mining._stubs import MiningConfig
...
def _ensure_config_dir() -> Path:
    ...
    from edith.security.file_utils import secure_mkdir
...
def recommend_model(hw: HardwareInfo, engine: str) -> str:
    ...
    """
    from edith.intelligence.model_catalog import BUILTIN_MODELS
...
def validate_config_key(dotted_key: str) -> type:
    ...
    for i, part in enumerate(parts):
        ...
        if isinstance(fld_type, str):
            # Evaluate forward references in the config module namespace safely
            import edith.core.config as _cfg_mod

            clean_type = fld_type.replace("Optional[", "").replace("]", "").replace('"', '').replace("'", "")
            fld_type = getattr(_cfg_mod, clean_type, fld_type)
...
def _parse_mining_section(data: dict) -> Optional["MiningConfig"]:
    ...
    from edith.mining._stubs import MiningConfig, PoolTarget, SoloTarget
