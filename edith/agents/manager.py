...
class AgentManager:
    ...
    def start_tick(self, agent_id: str) -> None:
        ...
        with self._lock:
            cur = self._conn.execute(
                "UPDATE managed_agents SET status = 'running', updated_at = ? "
                "WHERE id = ? AND status != 'running'",
                (time.time(), agent_id),
            )
            self._conn.commit()
        if cur.rowcount == 0:
            raise ValueError(f"Agent {agent_id} is already executing a tick or doesn't exist")
    ...
    def bind_channel(
        ...
        if binding is None:
            raise ValueError(f"Failed to create binding {binding_id}")
        return binding
    ...
    def list_templates() -> List[Dict[str, Any]]:
        ...
        # Built-in templates
        try:
            tpl_dir = importlib.resources.files("edith.agents") / "templates"
