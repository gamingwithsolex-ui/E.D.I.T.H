...
from edith.skills.dependency import validate_dependencies
from edith.skills.executor import SkillExecutor, SkillResult
from edith.skills.loader import discover_skills
from edith.skills.tool_adapter import SkillTool
from edith.skills.types import SkillManifest
...
class SkillManager:
    ...
    def _load_overlays(self) -> None:
        ...
        examples under ``manifest.metadata.edith.few_shot``.

        Bad overlays are silently ignored — they should not break discovery.
        """
        from edith.skills.overlay import SkillOverlayLoader
        ...
        for name, manifest in self._skills.items():
            ...
            if overlay.few_shot:
                ...
                oj = dict(new_metadata.get("edith", {}) or {})
                oj["few_shot"] = list(overlay.few_shot)
                new_metadata["edith"] = oj
    ...
    def get_few_shot_examples(self) -> List[str]:
        ...
        Pulls from ``manifest.metadata.edith.few_shot`` for every
        registered skill.  Returns one formatted string per example.
        """
        examples: List[str] = []
        for name, manifest in self._skills.items():
            oj = manifest.metadata.get("edith", {}) if manifest.metadata else {}
    ...
    def find_installed_paths(
        ...
        for root in roots:
            ...
            for candidate in root.rglob("*"):
                ...
                try:
                    from edith.skills.loader import load_skill_directory
