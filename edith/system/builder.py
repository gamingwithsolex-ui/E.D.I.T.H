...
from edith.core.config import JarvisConfig, load_config
from edith.core.events import EventBus, get_event_bus
from edith.engine._stubs import InferenceEngine
from edith.system.core import JarvisSystem
from edith.tools._stubs import BaseTool, ToolExecutor
...
class SystemBuilder:
    ...
    def build(self) -> JarvisSystem:
        ...
        if telemetry_enabled and config.telemetry.gpu_metrics:
            try:
                from edith.telemetry.energy_monitor import (
            ...
            if energy_monitor is None:
                try:
                    from edith.telemetry.gpu_monitor import GpuMonitor
        ...
        from edith.security import setup_security
        ...
        if agent_manager is not None:
            try:
                ...
                from edith.agents.scheduler import AgentScheduler
        ...
        if speech_enabled:
            try:
                from edith.speech._discovery import get_speech_backend
    ...
    def _resolve_engine(self, config: JarvisConfig):
        from edith.engine._discovery import get_engine
    ...
    def _setup_telemetry(self, config, bus):
        try:
            from edith.telemetry.store import TelemetryStore
    ...
    def _resolve_memory(self, config):
        try:
            import edith.tools.storage  # noqa: F401 -- trigger registration
            from edith.core.registry import MemoryRegistry
    ...
    def _resolve_channel(self, config, bus):
        ...
        try:
            import edith.channels  # noqa: F401 -- trigger registration
            from edith.core.registry import ChannelRegistry
            from edith.system._channel_kwargs import build_channel_kwargs
    ...
    def _resolve_tools(
        ...
        from edith.mcp.server import MCPServer
    ...
    def _setup_sandbox(self, config):
        ...
        if not sandbox_enabled:
            return None
        try:
            from edith.sandbox.runner import ContainerRunner
    ...
    def _setup_scheduler(self, config, bus):
        ...
        try:
            ...
            from edith.scheduler.scheduler import TaskScheduler
    ...
    def _setup_learning_orchestrator(config: JarvisConfig):
        ...
        try:
            from edith.core.config import DEFAULT_CONFIG_DIR
            from edith.learning.learning_orchestrator import (
                LearningOrchestrator,
            )
            from edith.learning.training.lora import LoRATrainingConfig
            from edith.traces.store import TraceStore
