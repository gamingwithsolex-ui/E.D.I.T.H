...
from edith.agents.manager import AgentManager
...
logger = logging.getLogger("edith.server.agent_manager")
...
def _make_lightweight_system(
    ...
    """
    try:
        from edith.engine.ollama import OllamaEngine
        ...
        try:
            from edith.core.events import get_event_bus
            from edith.telemetry.instrumented_engine import (
                InstrumentedEngine,
            )
...
def _ensure_registries_populated() -> None:
    ...
    from edith.core.registry import ChannelRegistry, ToolRegistry

    # First, try a normal import (works if modules haven't been imported yet)
    try:
        import edith.channels  # noqa: F401
    except Exception:
        pass

    try:
        import edith.tools  # noqa: F401
    except Exception:
        pass

    # Also try to import browser tools (not included in edith.tools.__init__)
    for _browser_mod in ("edith.tools.browser", "edith.tools.browser_axtree"):
        try:
    ...
    if not ChannelRegistry.keys():
        for mod_name in list(sys.modules):
            if mod_name.startswith("edith.channels.") and not mod_name.endswith(
                "_stubs"
            ):
                try:
    ...
    if not ToolRegistry.keys():
        for mod_name in list(sys.modules):
            if (
                mod_name.startswith("edith.tools.")
...
def build_tools_list() -> List[Dict[str, Any]]:
    ...
    from edith.core.credentials import TOOL_CREDENTIALS
    from edith.core.registry import ChannelRegistry, ToolRegistry
...
def _build_deep_research_tools(
    ...
    if not knowledge_db_path:
        from edith.core.config import DEFAULT_CONFIG_DIR
    ...
    from edith.connectors.retriever import TwoStageRetriever
    from edith.connectors.store import KnowledgeStore
    from edith.tools.knowledge_search import KnowledgeSearchTool
    from edith.tools.knowledge_sql import KnowledgeSQLTool
    from edith.tools.scan_chunks import ScanChunksTool
    from edith.tools.think import ThinkTool
...
def _get_mcp_tools(app_state: Any) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    from edith.mcp.client import MCPClient
    from edith.mcp.transport import StdioTransport, StreamableHTTPTransport
    from edith.tools.mcp_adapter import MCPToolProvider
...
async def _stream_managed_agent(
    *,
    ...
    from edith.core.types import Message, Role
    ...
    if agent_type == "deep_research" and app_state is not None:
        ...
        if dr_tools:
            ...
            async def generate_deep_research():
                ...
                from edith.agents.deep_research import DeepResearchAgent
    ...
    async def generate():
        ...
        while turns < max_turns:
            ...
            if tool_call_fragments and current_finish_reason == "tool_calls":
                ...
                from edith.core.types import ToolCall as MsgToolCall
                ...
                for tc in sorted_tcs:
                    ...
                    try:
                        ...
                        if mcp_adapter is not None:
                            ...
                            from edith.core.registry import ToolRegistry
                            from edith.tools._stubs import (
...
def create_agent_manager_router(
    ...
    async def run_agent(agent_id: str, request: Request):
        ...
        def _run_tick():
            try:
                from edith.agents.executor import AgentExecutor
                from edith.core.events import get_event_bus
    ...
    async def bind_channel(
        ...
        if req.channel_type == "imessage":
            ...
            if identifier:
                try:
                    from edith.channels.imessage_daemon import (
                        is_running,
                        run_daemon,
                    )
                    ...
                    if not is_running():
                        ...
                        if engine:
                            from edith.server.agent_manager_routes import (
                                _build_deep_research_tools,
                            )

                            tools = _build_deep_research_tools(engine=engine, model="")
                            if tools:
                                from edith.agents.deep_research import (
                                    DeepResearchAgent,
                                )
        ...
        if req.channel_type == "sendblue":
            ...
            if api_key_id and api_secret_key:
                try:
                    from edith.channels.sendblue import (
                        SendBlueChannel,
                    )
                    ...
                    if bridge and hasattr(bridge, "_channels"):
                        ...
                        from edith.server.channel_bridge import (
                            ChannelBridge,
                        )
                        from edith.server.session_store import (
                            SessionStore,
                        )
        ...
        if req.channel_type == "slack":
            ...
            if bot_token and app_token:
                try:
                    from edith.channels.slack_daemon import (
                        start_slack_daemon,
                    )
                    from edith.channels.slack_daemon import (
    ...
    def trigger_learning(agent_id: str):
        if not manager.get_agent(agent_id):
            raise HTTPException(status_code=404, detail="Agent not found")
        from edith.core.events import EventType, get_event_bus
    ...
    async def save_tool_credentials(tool_name: str, request: Request):
        from edith.core.credentials import save_credential
    ...
    def credential_status(tool_name: str):
        from edith.core.credentials import get_credential_status
