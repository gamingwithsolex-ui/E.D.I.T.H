...
async def list_agents(request: Request):
    ...
    registered = []
    try:
        import edith.agents  # noqa: F401 — side-effect registration
    ...
    running = []
    try:
        from edith.tools.agent_tools import _SPAWNED_AGENTS
...
async def create_agent(req: AgentCreateRequest, request: Request):
    """Spawn a new agent."""
    try:
        from edith.tools.agent_tools import AgentSpawnTool
...
async def kill_agent(agent_id: str, request: Request):
    ...
    try:
        from edith.tools.agent_tools import AgentKillTool
...
async def message_agent(agent_id: str, req: AgentMessageRequest, request: Request):
    ...
    try:
        from edith.tools.agent_tools import AgentSendTool
...
def _get_memory_backend(request: Request):
    ...
    if backend is None:
        try:
            from edith.tools.storage.sqlite import SQLiteMemory
...
async def telemetry_stats(request: Request):
    ...
    try:
        ...
        from edith.telemetry.aggregator import TelemetryAggregator
...
async def list_skills(request: Request):
    ...
    try:
        from edith.core.registry import SkillRegistry
...
async def list_sessions(request: Request, limit: int = 20):
    ...
    try:
        from edith.sessions.store import SessionStore
...
async def learning_stats(request: Request):
    ...
    # Skill discovery
    try:
        from edith.learning.agents.skill_discovery import SkillDiscovery
...
def include_all_routes(app) -> None:
    ...
    from edith.server.approval_routes import (
    ...
    try:
        if hasattr(app.state, "agent_manager") and app.state.agent_manager:
            from edith.server.agent_manager_routes import (  # noqa: PLC0415
    ...
    try:
        ...
        from edith.server.ws_bridge import create_ws_router
